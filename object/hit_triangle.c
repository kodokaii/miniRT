...
/*   Updated: 2024/02/03 13:24:40 by nlaerema         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	_set_uv(t_ray *ray, t_hit *hit)
{
	...
	hit->uv[X] = kdm_vec3_dot(delta, ray->object->x) / TRIANGLE_UV_SIZE;
	hit->uv[Y] = kdm_vec3_dot(delta, ray->object->y) / TRIANGLE_UV_SIZE;
	if (hit->uv[X] < 0.0f)
